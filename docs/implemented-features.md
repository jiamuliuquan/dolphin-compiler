...
M13 引入结构体和枚举，让类型系统能够表达复合数据和带状态的分支逻辑。值传递、移动/复制语义不在本里程碑内决定，统一延后到 M14 内存模型提案。
...
- 结构体支持作为局部变量和 `var`/`val` 赋值；`var` 声明的结构体整体可重新赋值，但字段不可单独写入（字段写入语义随 M14 内存模型确定）。
...
- `match` 作为表达式时各分支求值为同一类型；作为语句时忽略结果。
...
- 结构体和枚举**可以作为**函数参数或返回值（M14 起按值传递，浅拷贝 + 显式管理）。
- 结构体/枚举支持作为局部变量、整体赋值、字段访问和 `match` 主题。
- 枚举 payload 字段支持基础标量与结构体；不支持 `string` 作为枚举 payload 字段。

## 13. 内存模型与动态数据（M14）

M14 首次为语言引入堆，内存模型冻结为 **Zig 式显式控制**：值语义 + 显式指针 + 手动分配，不采用所有权、引用计数或垃圾回收。

### 13.1 显式指针

`*T` 是指针类型，`&expr` 取址，`*expr` 解引用，`q->x` 等价于 `(*q).x`：
...
- 指针是值类型，赋值/传参复制地址。
- 首版最小模型：指针默认非空（声明必须初始化）；不做悬垂检查；不支持指针算术；`&` 仅作用于局部变量。
- 指针可指向标量、结构体或枚举。

### 13.2 动态切片

`[]T`（首版仅 `[]u8`）是 `{ ptr, len }` 二元组，与 `string` 别名等价（同一布局）：

```dc
var buf = allocate(4);
buf[0] = 65_u8;          // 索引写入
val first = buf[0];      // 索引读取（带运行时越界检查）
val len = length(buf);   // 长度
free(buf);               // 释放
```

- `allocate(n: 整数) -> []u8` 分配 `n` 字节；`free(s)` 释放。
- 切片是值类型，复制为浅拷贝（共享底层数据）。
- `string` 保持不可索引约束，`[]u8` 支持索引；二者在 `free`/`allocate` 通道、ABI 与内存布局上等价。

### 13.3 确定性释放：`defer` 与 `try`

`defer <expr>` 把一条语句推迟到所属块作用域退出时执行（含提前 `return`）；多个 `defer` 按声明顺序逆序执行：
...
`try (var a = allocate(..), var b = allocate(..)) { ... }` 是「`defer free` + 块」的语法糖，资源出块自动逆序释放，且**禁止逃逸**（重新绑定、赋给块外变量、返回均为编译期错误）。

### 13.4 字符串拼接

`a + b` 是唯一隐式分配点：展开为隐式 `allocate` + 拼接，返回新 `string`，释放责任交接收者：
...
Debug 构建启用检测分配器：双重释放触发退出码 `103`；程序退出时未释放的块报告到 stderr（泄漏检测）。分配失败恒触发退出码 `102`。

> 实现取舍：运行时目标文件在编译编译器时一次性内嵌，无法按 Debug/Release 分别链接，因此检测分配器始终启用（见提案 §12.6/§12.9）。
...
dolphin_allocate
dolphin_free
dolphin_string_concat
dolphin_runtime_exit
```
...
- 指针解引用非指针、`->` 作用于非结构体指针、对非变量取址。
- `free`/`allocate` 参数类型错误。
- `try` 资源逃逸（重新绑定、赋给块外变量、返回）。
