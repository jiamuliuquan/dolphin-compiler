...
> 状态：H19-00 设计冻结产物。**本文描述的 API 均未实现**；在 H19-01 起按批次落地前，
> 不能把本文条目写进“已实现功能”。实现顺序与验收编号见 [M18-M21 计划](plan-m18-plus.md) 第 5 节
> 与本文“测试矩阵”。
>
> 前置：M18 已完成并通过阶段验收（[m18-progress](reports/m18-progress.md)）。
>
> 决策请求：D1–D3 需用户确认后才能开始 H19-01（见第 12 节）。确认前 H19-01 及后续批次**阻塞**。
...
| 编号 | 请求 | 推荐 |
| --- | --- | --- |
| D1 | `dc build --lib` 是否保持“产出目标文件 + 打包 `.dlib`”不变，M19 仅新增 `dc test` 作为开发/测试闭环；不放宽 `.dlib` 的 path 依赖限制 | 保持不变（第 8 节） |
| D2 | 测试发现与运行约定：`tests/*.do` + `test_*` 自动发现（不新增 `[[test]]` 清单字段）、`target/test/` 输出、断言失败退出码 `106`、0 测试/过滤无匹配退出 1 | 按第 9 节冻结 |
| D3 | M19 不新增语法（含 match 语句块）；保留 M14 defer 语义，用句柄状态机与显式 `close`/`release` 处理重绑定 | 按第 10 节冻结 |

未确认前：**H19-01 及之后批次阻塞**；可以只做不依赖 D1–D3 的准备（阅读源码、写失败回归设计），
不得开始实现。
...
- **CLI**：`dc test` 为新增子命令；`dc run` 增加 `--` 之后的应用参数（增量语法，原有调用不变）。
