...
> 设计提案：见 [M15 设计提案](proposal-m15-generics-stdlib.md)。泛型走**单态化**，契约关键字定为 `trait`，产出类型用**关联类型**表达。

目标：在 M14 内存模型定型后引入泛型，并据此构建不依赖编译器特判的标准库。

- [x] 泛型函数和泛型数据类型（单态化，跨模块稳定 mangling）
- [x] trait/interface、方法、关联函数与关联类型（`type Item` / `Self::Item`）
- [x] `Option<T>`、`Result<T, E>`（`?` 错误传播语法明确延后，见提案 §8）
- [x] 迭代器（`Iterator` 契约 + `for x in iterable` 泛型化）、字符串工具、进程样板（`run`）
- [ ] 集合（`Vec<T>` 等自动扩容容器：类型擦除延后，见提案 §6.2.1a）
- [ ] 文件 I/O（句柄/缓冲 ABI 延后，见提案 §6.2.2）

验收目标：类型参数参与名称解析、类型检查和代码生成；标准库不依赖编译器对每个具体类型进行硬编码。

> 延后项（已在提案记录，不阻塞 M15 完成）：`Vec<T>`（类型擦除）、`Range` 下沉、无参枚举项作为表达式（跨模块）、`?` 运算符、文件 I/O。
