...
> 状态：H20-00 设计冻结产物。**本文冻结的 API、格式与语义已由 H20-01..05 实现并通过
> 三平台默认 lane 与 Linux/macOS LLVM Debug 验收**（Windows H20-W、macOS H20-M 平台复验，
> 见 [M20 报告](reports/m20-progress.md)）；本文继续作为 M20 的规范与验收矩阵。
...
>
> 决策请求：D-M20-1..4 已于 2026-09-24 由用户确认（第 13 节）；实现过程中的平台差异
> （Windows `\\?\`、macOS 符号链接路径）已按用户确认以词法绝对路径修复（H20-W/H20-M）。
...
- **实现状态**：H20-01..05 已完成并验收；本文 API 与行为已写入当前能力文档（[已实现功能参考](implemented-features.md) 18.5）与 README。
...
1. **H20-01..05 已全部完成**：D-M20-1..4 已于 2026-09-24 确认，批次按第 12 节执行完毕；
   三平台默认 lane 与 Linux/macOS LLVM Debug 证据见 [M20 报告](reports/m20-progress.md)。
2. **H20-05 工具可用性**：Linux gdb 17.2（H20-05）与 macOS lldb-2100（H20-M）均已实测；
   Windows/PDB 按 §9.1 不在本阶段范围。
3. **函数体内多错误收集**：M20 明确不做（§5.1）；若后续真实项目证明必需，另开决策与批次，
   不在本阶段扩围。
4. **表达式级类型/局部变量值**：不在 M20；hover 与调试器只提供声明信息，若用户要求更强
   能力需新规格。
5. 本文冻结的 API 与行为已实现并验收；M20 阶段完成状态以 [M20 报告](reports/m20-progress.md)
   的 H20-M 节结论与 [已实现功能参考](implemented-features.md) 为准。
