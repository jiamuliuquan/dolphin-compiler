...
> 当前基线：M0-M8 MVP 已完成，M9-M11 已完成，M12 核心链路（内嵌运行时 + LLD）已落地，发行包流水线进行中
...
验收目标：容器能够跨函数和模块安全使用；在定义好的安全子集中不发生泄漏、重复释放或悬垂引用。采用所有权、引用计数或垃圾回收，应由独立设计提案决定。
...
- [ ] 增加 DWARF/PDB 调试信息、格式化器和语言服务器的最小可用版本
- [ ] 在收益明确后再设计增量编译和构建缓存
