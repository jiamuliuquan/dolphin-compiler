...
从 `target/release` 收集 `dc` 与 `dolphin-compiler`，连同 LICENSE 与 README
组装成发行包（Linux/macOS 为 .tar.gz，Windows 为 .zip），并生成 SHA-256
校验和。

发行包不再携带链接器：默认使用系统链接器（Unix `cc`，Windows `link`），
需要 `rust-lld` 时用 `--bundled-linker` 从本机 Rust 工具链解析。
