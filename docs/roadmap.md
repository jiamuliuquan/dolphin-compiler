...
> 当前基线：M0-M14 已完成，M15-A/M15-B/M15-C/M15-D/M15-E/M15-F 已完成；M15 全部批次通过验收。M16 已完成：后端无关类型化 IR、可选 LLVM 后端（Cranelift 保留为快速编译后端）与基准。M17 已完成：DWARF 调试信息（LLVM 后端 Debug）、`dc fmt` 格式化器与 `dc lsp` 语言服务器。旧 M14/M15 实现已清理，M14 已从 M13 重新实现并通过验收。

路线图以[已实现功能参考](implemented-features.md)为基线，以[语言设计说明](language-design.md)为目标。只有代码、测试、示例和文档全部完成，里程碑才能标记为完成。

后续执行见 [M18-M21 人工交接指南](plan-m18-plus.md)与 [M18 正确性合同](plan-m18-correctness.md)。M18 尚未实施；M19/M20 先做设计冻结，M21 按实际负载与交付需求规划。M0-M17 的完成标记保留为历史交付记录，不代表已经排除所有组合语义缺陷；2026-09-17 审计复现与验证范围见 M18 合同。
...
原验收标准还要求新机器只需操作系统组件与发行包即可 `dc build`。三个目标的打包与开发 runner 冒烟已存在，但上述无开发环境承诺尚未通过隔离 CRT/SDK 的验收。M18 补发布质量门禁，M21 冻结系统依赖策略并验证干净环境；当前要求以[安装说明](installation.md)为准。
...
延后项：版本范围求解、闭源二进制 Dolphin 包、完整仓库网站后端、自定义 allocator、文件/进程/网络完整标准库、动态多态、`?T`/`?`。`Vec<T>` 与精确版本包管理已经纳入本次验收。
...
原验收目标：调试信息可被主流调试器加载；格式化器和语言服务器对 M1-M15 的全部语法特性可用。实际交付范围以上述 MVP 条目为准：LSP 不具备项目级语义分析/跨文件导航；DWARF 自动测试目前主要检查调试段，不等于真实调试器完整验收。项目级工具、格式化保持性和调试器实测纳入 M20，不以原目标措辞扩大当前承诺。
...
- [ ] 聚合写入/求值顺序、枚举布局、双后端饱和转换正确性
- [ ] 类型声明 bound、impl 头校验与不支持语法的明确拒绝
- [ ] 后端无关 IR verifier 与 LLVM module verification
- [ ] 包来源冲突、依赖目标加载与 profile 优先级回归
- [ ] LLVM Linux CI、同提交发布门禁、真实 stderr/泄漏检查
- [ ] 当前文档、可运行示例和组合回归验收
...
- [ ] 先冻结 API、资源/错误状态、测试目标及公开行为兼容决策
- [ ] 参数/环境、dc run 参数转发、基础字节 I/O 与文件 API
- [ ] 最小文本/数字处理、Result/defer 错误路径
- [ ] 用户 dc test 与库开发闭环
- [ ] Dolphin 编写的真实文件/标准输入处理工具与自动化验收

验收：`examples/m19` 能接收真实输入、报告错误、自动测试并清理资源；不依赖外部脚本实现主要应用逻辑，不以异步/宏/闭包为前置。

### M20：项目级开发工具

- [ ] 结构化诊断与有限错误恢复
- [ ] 无构建/网络副作用的共享项目分析与未保存文件 overlay
- [ ] stdlib/依赖/lib/多 bin 项目的准确诊断、悬停和定义导航
- [ ] 格式化幂等/语义保持、项目源码发现
- [ ] 真实调试器断点/单步/调用栈与项目开发流程验收

验收：M19 项目使用依赖与未保存文档时仍能被正确分析；不把段名存在当作调试体验完成，不自动纳入 PDB/Cranelift 完整调试。
...
验收：性能结论有输入、原始数据和回归预算；实际上传的发行包满足声明环境，工具链升级/离线消费行为明确。构建缓存是候选实现，不为勾选功能而强行引入增量框架。

## 8. 暂不排期的候选能力

以下能力保留为方向，不自动纳入 M18-M21 完成条件；重新评估条件见交接指南。
