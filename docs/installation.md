...
Dolphin 编译器以携带预编译 Dolphin 运行时的发行包发布，使用发行包不需要编译器源码或 Rust；发行包不再携带链接器，默认使用系统链接器（Unix `cc`，Windows `link`），因此本机需要开发工具链。能否在目标机器构建并链接程序受[系统依赖边界](#6-系统依赖边界)限制，尚不承诺任意无开发工具链、无 SDK 的干净机器解压即可构建。
...
发行包不含链接器：默认链接由系统提供（Unix `cc`，Windows 已激活 MSVC 开发环境中的 `link`）。装有 Rust 工具链时，`--bundled-linker` 会从 PATH 或 `rustc --print sysroot` 解析并使用 `rust-lld`。

官方默认 feature 只启用 Cranelift codegen，不启用 LLVM codegen；发行包不再携带任何 LLVM 动态库，打包行为见 [`scripts/package.py`](../scripts/package.py)。
...
在所需本地系统链接依赖齐备时，发行包可离线使用：`dc check/build/run/test` 在不声明远程依赖时不访问网络。Dolphin 运行时目标文件内嵌于 `dc`，链接由本机系统链接器完成；离线不代表缺少 CRT/SDK 时也能完成链接。
...
应区分三层依赖：运行 `dc`、链接 Dolphin 程序、运行最终程序。预编译 Dolphin 运行时免除了用户现场编译 runtime；默认链接直接调用系统链接器（Unix `cc`，Windows `link`），因此需要本机开发工具链与链接所需的系统 CRT 对象、导入库或 SDK。`--bundled-linker` 可改用 Rust 工具链自带的 `rust-lld`，但它同样不提供这些系统链接文件。
...
构建**编译器自身**（从源码 `cargo build`）需要 Rust 和本机 C/C++ 编译工具链及对应平台链接依赖。可选 LLVM 后端（`cargo build --features llvm`）还要求本机安装 LLVM 开发库（`llvm-config` 可被 `llvm-sys` 定位，当前对接 LLVM 22）。默认 Cranelift codegen 不需要 LLVM 开发库，发行包也不再携带任何 LLVM 动态库。以 `--features llvm` 自行构建的 `dc` 若动态链接 LLVM，运行环境需具备匹配的库；以实际产物的依赖检查为准。
...
- `--color`、`--debug`/`--release`、`--bin`、`--lib`、`--output`、`--system-linker`、`--bundled-linker`、`--backend`、`--locked`、`--offline`、`--repository`、`--filter` 为稳定选项；`dc run` 的 `-- <应用参数>` 原样转发语义同样稳定。
...
| --- | --- |
| 编译器 `dc` 体积 | 约 6.3 MB |
| 发行包体积（不含链接器） | 约 9 MB（Linux x86_64 实测；此前随包携带 `rust-lld` 时约 114 MB） |
