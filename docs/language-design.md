...
> 本章描述第三阶段（M13-M15）计划引入的数据类型与内存模型。18.1-18.3 的**结构体、枚举与 `match` 已在 M13 实现并冻结语法**；18.4 的内存模型仍为设计草案。当前编译器的真实能力以[已实现功能参考](implemented-features.md)为准。值传递、移动/复制与所有权规则以 M14 独立设计提案为准。

### 18.1 结构体（M13 已实现）

结构体把同名字段聚合为一个复合值类型，采用位置构造与字段访问；按值传递的最终语义（复制或移动）延后到 M14 内存模型提案确定。
