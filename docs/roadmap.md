...
- [ ] 结构体、字段访问、构造（不可变优先，值传递语义留待 M14）
- [ ] 枚举、携带数据的枚举项
- [ ] `match` 模式匹配
- [ ] 基于枚举实现非泛型的错误处理样例，验证 ABI 与控制流设计
...
- [ ] 先确定值语义、移动/复制规则、析构时机和字符串所有权
- [ ] 回溯 M13 的结构体，补齐按值传递的最终语义
- [ ] 引用或借用的最小模型，以及切片
- [ ] 动态字符串和动态数组
- [ ] 越界、容量溢出、分配失败和释放规则

验收目标：容器能够跨函数和模块安全使用；在定义好的安全子集中不发生泄漏、重复释放或悬垂引用。采用所有权、引用计数或垃圾回收，应由独立设计提案决定；提案通过后，M13 已实现的结构体按最终语义对齐，不留半成品。
