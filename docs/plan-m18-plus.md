...
前置：M18 完成。产出 `docs/proposal-m19-cli-stdlib.md`（计划新增文件）和 `docs/reports/m19-progress.md`，再执行后续代码批次。
...
| 编号 | 工作 | 前置 | 状态 |
| --- | --- | --- | --- |
| H19-00 | API/目标程序/兼容决策冻结 | M18 | 待实施 |
| H19-01 | 应用参数、环境、dc run 转发 | H19-00 | 待实施 |
| H19-02 | 标准流与字节 I/O | H19-01 | 待实施 |
| H19-03 | 文件操作与资源错误路径 | H19-02 | 待实施 |
| H19-04 | 必要文本/数字处理 | H19-03 | 待实施 |
| H19-05 | 最小 dc test 与库开发闭环 | H19-04 | 待实施 |
...
| 编号 | 工作 | 前置 | 状态 |
| --- | --- | --- | --- |
| H20-00 | 项目分析与工具协议规格 | M19 | 待实施 |
| H20-01 | 结构化诊断与有限恢复 | H20-00 | 待实施 |
...
M21 是条件规划：必须用 M19/M20 的实际项目与测量来选优化，不提前承诺某个框架。可先完成测量和兼容设计，再按批准结果实施。
