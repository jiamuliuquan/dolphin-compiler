...
/// 用户声明的原生链接输入（M14-E，来自 `dolphin.toml` 的 `[native.<triple>]`）。
///
/// 路径均为绝对路径，且已按清单所在目录解析。`runtime_files` 是构建 bin 时
/// 需要复制到可执行文件同目录的运行时附件（如随程序分发的 `.dll`/`.so`）。
...
pub trait TargetPlatform {
    ...
    ///
    /// M14-C/R04 起有两份：`debug` 为 true 时返回带存活分配登记表与泄漏报告的
    /// 检测版，否则返回普通版。由 `BuildProfile` 决定链接哪一份。
    fn runtime_object_bytes(&self, debug: bool) -> &'static [u8];
...
impl TargetPlatform for UnixPlatform {
    ...
    fn runtime_object_bytes(&self, debug: bool) -> &'static [u8] {
        if debug {
            include_bytes!(concat!(env!("OUT_DIR"), "/dolphin_runtime_object_debug"))
        } else {
            include_bytes!(concat!(env!("OUT_DIR"), "/dolphin_runtime_object_release"))
        }
    }
    ...
    fn link_command(
        ...
        let append_native = |command: &mut Vec<OsString>| {
            command.extend(
                native
                    .objects
                    .iter()
            ...
            if !native.shared_libs.is_empty() {
                ...
                if self.triple.operating_system.is_like_darwin() {
                    command.push(OsString::from("@loader_path"));
                } else {
                    command.push(OsString::from("$ORIGIN"));
                }
            }
        };
...
impl TargetPlatform for WindowsPlatform {
    ...
    fn link_command(
        ...
        command.push(OsString::from("/SUBSYSTEM:CONSOLE"));
        command
    }
...
mod tests {
    ...
    fn unix_link_command_uses_rust_lld_gnu_flavor() {
        ...
        let command = platform.link_command(object, runtime, &NativeInputs::default(), output);
    ...
    fn unix_system_link_command_uses_cc() {
        ...
        let command =
            platform.system_link_command(object, runtime, &NativeInputs::default(), output);
        assert_eq!(
            command,
            vec![
                OsString::from("cc"),
    ...
    fn embedded_runtime_object_is_non_empty() {
        let platform = host().expect("host platform should be supported");
        assert!(
            !platform.runtime_object_bytes(true).is_empty(),
            "embedded debug runtime object should be non-empty"
        );
        assert!(
            !platform.runtime_object_bytes(false).is_empty(),
            "embedded release runtime object should be non-empty"
        );
    }
