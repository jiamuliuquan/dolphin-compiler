...
> 对应实现记录：M0-M17；M18 正确性收敛已完成（H18-00..11，证据见 [M18 进度报告](reports/m18-progress.md)）
>
> 编译器目录：[`仓库根目录`](../)  
> 可运行示例：[`examples`](../examples/)

当前源码已实现到 M17。本文记录已有能力、明确限制和已知缺陷，不把历史里程碑完成标记当作无缺陷证明。后续任务见 [M18-M21 交接指南](plan-m18-plus.md)及 [M18 执行合同](plan-m18-correctness.md)；M14/M15 旧实施指南已经归档，不是当前待办。
